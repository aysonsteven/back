...
class Post_Data_Interface extends Post_Data {
    ...
    public function create( $record = [] ) {
        ...
        */
        ...
        // @todo move it to validator
        if( isset($record['Supplier']) && strlen( $record['Supplier'] ) > 254 ) return error( ERROR_TITLE_TOO_LONG );
        ...
        if( ! in('AmountDue') ) return success( ['idx'=>$post_idx, 'Supplier' => in('Supplier'), 'TIN'=> in('TIN')] );
        if( in('AmountDue')) return success( ['idx'=>$post_idx, 'AmountOfPurchase' => in('AmountOfPurchase'), 'InputTax'=> in('InputTax'), 'AmountDue' => in('AmountDue'), 'InvoiceNo' => in('InvoiceNo')] );
