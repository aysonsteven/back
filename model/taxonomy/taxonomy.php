...
//        debug_log($option);
        ...
        if ( in('limit')  ) {
            if ( is_numeric( in('limit') ) ) $limit = in('limit');
        ...
        if ( ! $limit ) $q = "SELECT $option[select] FROM {$this->getTable()} $where $order";
        else $q = "SELECT $option[select] FROM {$this->getTable()} $where $order $limit";
